...
# Log all actions
log_file="/root/fortress_developer_log_$(date +%F_%T).txt"
echo "Starting fortress developer script at $(date)" | tee -a "$log_file"

# Enhanced CLI Title with Emoji and Yellow Color (Inserted here, line 11)
...
handle_error() {
    local line=$1
    echo "Error on line $line: $2" | tee -a "$log_file"
    # Attempt recovery
    if command -v apt >/dev/null 2>&1; then
        apt-get --fix-broken install -y || echo "Recovery failed" | tee -a "$log_file"
    else
        echo "Apt unavailable, manual intervention required" | tee -a "$log_file"
        exit 1
    fi
}
...
apt-get update || echo "Update failed, proceeding" | tee -a "$log_file"
apt-get --fix-broken install -y || dpkg --configure -a

# Step 2: Define trusted package whitelist (expanded for desktop and dev tools)
...
trusted_packages=(
    ...
    "rustc" "cargo" # Rustup replaced with cargo for simplicity
    # Security tools
...
apt install -y "${trusted_packages[@]}" || echo "Some installs failed, proceeding" | tee -a "$log_file"
...
# Step 4: Purge only injected packages (preserve desktop essentials)
...
# Step 5: Remove risky binaries (exclude trusted tools)
...
# Step 6: Clean system directories and GRUB modules
...
# Step 7: Compile secure Linux 6.14-rc5 kernel with hardware optimization and PQ ciphers
...
cd linux-6.14-rc5
# Custom config for hardware, security, and AI scheduler (inspired by Arighi)
cat << EOF > .config
# Core system
CONFIG_SMP=y
CONFIG_NR_CPUS=12
...
CONFIG_X86_64=y
# Security features
CONFIG_MODULES=y # Needed for desktop functionality
CONFIG_MODULE_SIG=y
...
# Hardware support (Gigabyte B760M-H-DDR4, i7-12700, GTX 970)
CONFIG_VIRTIO=n
CONFIG_VMX=y
CONFIG_VT=y
CONFIG_VTD=y
...
CONFIG_ETHERNET=y
CONFIG_NETDEVICES=y
CONFIG_E1000E=y # Intel I219-V Ethernet
CONFIG_USB=n
CONFIG_NVME_CORE=y
...
make -j12 bzImage || echo "Kernel build failed" | tee -a "$log_file"
make -j12 modules || echo "Modules build failed" | tee -a "$log_file"
...
# Step 8: Configure network renaming (net101)
echo "Renaming Ethernet to net101..." | tee -a "$log_file"
cat << EOF > /etc/udev/rules.d/70-persistent-net.rules
SUBSYSTEM=="net", ACTION=="add", DRIVERS=="e1000e", NAME="net101"
EOF
...
insmod ext2
set root='hd0,msdos1'
menuentry "Fortress Developer (Secure)" --users "admin" {
...
# Step 12: Configure and enable security tools
...
echo "Fortress Developer complete. Check $log_file. Test ISO in a VM." | tee -a "$log_file"
